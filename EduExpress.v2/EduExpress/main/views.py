...
class HomePageView(View):
	...
	def post(self, request):
		...
		messages.success(request,f"{client_request.name}, {_('Your message was successfully submitted, please wait our call !')}")
		telegram_bot_sendtext(f"  ClientMessage  \n Name: {client_request.name} \n Lang Certificate :  {client_request.lang_certificate}  \n Phone: +{client_request.phone} \n Link: https://www.eduexpress.info/admin/main/clientrequest/{client_request.id}/change")
		return redirect(f'/')
...
class UniversityDetailView(View):
    ...
    def post(self, request, slug):
        univ_id = request.POST.get("univ_id")
        # university = University.objects.get(id=int(univ_id
        name = request.POST.get('name')
        language_certificate = request.POST.get('language_certificate')
        phone = request.POST.get('phone')
        additional = request.POST.get('message')
        client_request = ClientRequest.objects.create(university_id=univ_id,name=name, phone=phone, lang_certificate=language_certificate, additional=additional)
        messages.success(request,f"{client_request.name}, {_('Your message was successfully submitted, please wait our call !')}")
        telegram_bot_sendtext(f"  Application:   \n   University:{client_request.university.name}  \n Name: {client_request.name} \n Lang Certificate :  {client_request.lang_certificate}  \n Phone: +{client_request.phone} \n Link: https://www.eduexpress.info/admin/main/clientrequest/{client_request.id}/change")
        # telegram_bot_sendtext(f"  {request.POST.get('univ_id')}")
        return redirect('/')
...
def get_message(request):
    ...
    phone = request.POST.get("surname")
    message = request.POST.get("phone")
    telegram_bot_sendtext(f"  ClientMessage  \n Name: {name} \n Email :  {email}  \n Phone: +{phone} \n Message: {message}")
    messages.success(request, _(f"{name}, Your message has been sent, please wait our call !"))
    return redirect("/")
